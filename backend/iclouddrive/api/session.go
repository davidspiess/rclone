...
func (s *Session) SignInInit(ctx context.Context, appleID, password string) (*SignInProof, error) {
	srp, err := icloud.NewSRP()
	if err != nil {
		return nil, err
	}

	// Apple requires the SRP A value in /signin/init before returning
	// protocol details, so the client must be created without a password here.
	client, err := srp.NewClient([]byte(appleID), nil)
	if err != nil {
		return nil, err
	}
	...
	srpPassword, err := pbkdf2.Key(srp.Hash().New, string(encodedPassword), salt, responseInfo.Iteration, 32)
	if err != nil {
		return nil, err
	}

	// Set the SRP password now, before computing the M1 proof value.
	client.SetPassword(srpPassword)
	...
	return &SignInProof{
		C:  responseInfo.C,
		M1: m1,
		M2: srp.HashBytes(client.A(), m1, client.Key()),
	}, nil
}
