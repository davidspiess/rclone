...
func (s *Session) SignInInit(ctx context.Context, appleID, password string) (*SignInProof, error) {
	group, err := srp.GetGroup(2048)
	if err != nil {
		return nil, err
	}

	var srpPassword []byte
	noUsernameInX := srp.X(func(s *srp.SRP, identity, _, salt []byte) *big.Int {
		// iCloud ignores identity for x-calculation
		return s.HashInt(salt, s.HashBytes([]byte(":"), srpPassword))
	})

	srp, err := srp.NewSRP(crypto.SHA256, group, noUsernameInX)
	if err != nil {
		return nil, err
	}
